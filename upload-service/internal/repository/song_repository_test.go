...
func TestGetAllSongs(t *testing.T) {
	...
	_, err = testDB.ExecContext(ctx,
		`INSERT INTO songs (name, auditions, genre_id, date, name_on_minio) VALUES
...
func TestGetSongByID(t *testing.T) {
	...
	err = testDB.QueryRowContext(ctx,
		...
		"My Song", 1, genreID, "http://song-name_on_minio",
	).Scan(&songID)
	require.NoError(t, err)
	...
	assert.Equal(t, "http://song-name_on_minio", song.NameOfMinio)
}
...
func TestCreateSongWithArtists(t *testing.T) {
	...
	song := &model.Song{
		Name:        "My Song",
		GenreID:     genreID,
		NameOfMinio: "https://name_on_minio-to-song.com",
	}
...
func TestUpdateSong(t *testing.T) {
	...
	err = testDB.QueryRowContext(ctx,
		...
		"Old Name", genreID, "http://old-name_on_minio.com",
	).Scan(&songID)
	require.NoError(t, err)
	...
	err = repo.UpdateSong(ctx, &model.Song{
		...
		Name:        "New Name",
		GenreID:     genreID,
		NameOfMinio: "http://new-name_on_minio.com",
	})
	require.NoError(t, err)
	...
	assert.Equal(t, "http://old-name_on_minio.com", updated.NameOfMinio)
}
...
func TestGetArtistsBySongID(t *testing.T) {
	...
	err = testDB.QueryRowContext(ctx,
		"INSERT INTO songs (name, genre_id, name_on_minio) VALUES ($1, $2, $3) RETURNING song_id",
		"Test Song", genreID, "http://name_on_minio-to-song",
	).Scan(&songID)
	require.NoError(t, err)
...
func TestGetAlbumBySongID(t *testing.T) {
	...
	err = testDB.QueryRowContext(ctx,
		"INSERT INTO songs (name, genre_id, name_on_minio) VALUES ($1, $2, $3) RETURNING song_id",
		"Song X", genreID, "http://name_on_minio-x",
	).Scan(&songID)
	require.NoError(t, err)
...
func TestBatchInsertSongsToAlbum_TooManySongs(t *testing.T) {
	...
	for i := 0; i < 51; i++ {
		...
		err := tx1.QueryRowContext(ctx,
			"INSERT INTO songs (name, genre_id, name_on_minio) VALUES ($1, $2, $3) RETURNING song_id",
			fmt.Sprintf("Song %d", i+1), genreID, fmt.Sprintf("http://name_on_minio/%d", i+1),
		).Scan(&songID)
		require.NoError(t, err)
