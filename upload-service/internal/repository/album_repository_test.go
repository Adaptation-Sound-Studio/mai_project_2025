...
func TestGetSongsByAlbumID(t *testing.T) {
	...
	err = testDB.QueryRowContext(ctx, `
		INSERT INTO songs (name, auditions, genre_id, date, name_on_minio)
...
func TestBatchInsertSongsToAlbum(t *testing.T) {
	...
	for i := 1; i <= 2; i++ {
		...
		err := tx1.QueryRowContext(ctx,
			...
			fmt.Sprintf("Song %d", i), 100*i, 1, fmt.Sprintf("http://name_on_minio%d", i),
		).Scan(&id)
		require.NoError(t, err)
		songIDs = append(songIDs, id)
	}
	...
	var name_on_minioedIDs []int64
	for rows.Next() {
		var id int64
		require.NoError(t, rows.Scan(&id))
		name_on_minioedIDs = append(name_on_minioedIDs, id)
	}
	require.NoError(t, rows.Err())

	assert.ElementsMatch(t, songIDs, name_on_minioedIDs)
}
