...
func RegisterUserRoutes(r *mux.Router, h *handler.UserHandler, sessionMiddleware func(http.Handler) http.Handler, roleMiddleware func(allowedRoles ...string) func(http.Handler) http.Handler, apiKeyMiddleware func(http.Handler) http.Handler) {
	r.Handle("/users/role", apiKeyMiddleware(http.HandlerFunc(h.UpdateUserRole))).Methods("POST")
